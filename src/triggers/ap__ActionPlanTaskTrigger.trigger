trigger ap__ActionPlanTaskTrigger on Task ( after update, after delete , before delete) {
    ...
    if (trigger.isUpdate && trigger.isAfter){
		...
		ap__ActionPlansUtilities apUtil = new ap__ActionPlansUtilities();
		...
		//Call to ap__ActionPlansUtilities in order to proceed with creation of dependent Task
	    if( !closedTasks.isEmpty() ) {
	        ap__ActionPlansTaskTriggerUtilities.initDependentTaskWork( closedTasks);
	    }
	    //update status to in progress for AP Tasks
	     if( !inProgressTasks.isEmpty() ) {
	        ap__ActionPlansTaskTriggerUtilities.updateAPTasksStatus( inProgressTasks );
	    }
    	...
    	List<String> taskTempIds = new List<String>();	
		 ...
		 if (trigger.isAfter){
			...
			for( APTaskTemplate__c ta : [select  a.Id ,a.Action_Plan__c 
	   		...
	   		if (finalIds.size()>0){
	   			  if (ap__ProcessorControl.inBatchContext){
		        	//delete 
		        	delete [select aPT.id from APTaskTemplate__c aPT where aPT.id in : finalIds];
		       	}else{
		        	ap__ActionPlansTaskTriggerUtilities.deleteAPTasks( finalIds );
		        }
	   		}
   		}
   		if (trigger.isBefore){
	    	...
	    	for( Task t : trigger.old ){
	   		...
	   		ap__ActionPlansTaskTriggerUtilities.initDependentTasksAndReset( taskTempIds);
   		}
