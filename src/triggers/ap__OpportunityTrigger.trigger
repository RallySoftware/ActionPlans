trigger ap__OpportunityTrigger on Opportunity (after undelete, before delete) {
	...
	if ( trigger.isdelete ){
		...
		for( ActionPlan__c ap : [ select Id from ActionPlan__c where Opportunity__c in : cIds ] ){
   		...
   		if ( apIds.size() >0 ){  		
			ap__ActionPlansBatchDelete aPBatch = new ap__ActionPlansBatchDelete(apIds, Userinfo.getUserId());
	...
	if ( trigger.isUnDelete ){
		for( Opportunity o : trigger.new ){
   		...
   		try{
   			if(ap__ActionPlanObjectTriggerTest.isTest){
   				//throw dmlException
   				insert new Contact();	
   			}
   			//undelete aPs;
