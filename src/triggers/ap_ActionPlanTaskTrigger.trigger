trigger ap_ActionPlanTaskTrigger on Task ( after update, after delete , before delete) {
    ...
    if (trigger.isUpdate && trigger.isAfter){
		...
		ap_ActionPlansUtilities apUtil = new ap_ActionPlansUtilities();
		...
		//Call to ap_ActionPlansUtilities in order to proceed with creation of dependent Task
	    if( !closedTasks.isEmpty() ) {
	        ap_ActionPlansTaskTriggerUtilities.initDependentTaskWork( closedTasks);
	    }
	    //update status to in progress for AP Tasks
	     if( !inProgressTasks.isEmpty() ) {
	        ap_ActionPlansTaskTriggerUtilities.updateAPTasksStatus( inProgressTasks );
	    }
    	...
    	List<String> taskTempIds = new List<String>();	
		 ...
		 if (trigger.isAfter){
			...
			for( APTaskTemplate__c ta : [select  a.Id ,a.Action_Plan__c 
	   		...
	   		if (finalIds.size()>0){
	   			  if (ap_ProcessorControl.inBatchContext){
		        	//delete 
		        	delete [select aPT.id from APTaskTemplate__c aPT where aPT.id in : finalIds];
		       	}else{
		        	ap_ActionPlansTaskTriggerUtilities.deleteAPTasks( finalIds );
		        }
	   		}
   		}
   		if (trigger.isBefore){
	    	...
	    	for( Task t : trigger.old ){
	   		...
	   		ap_ActionPlansTaskTriggerUtilities.initDependentTasksAndReset( taskTempIds);
   		}
